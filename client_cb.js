const { OPCUAClient, DataType, makeBrowsePath, AttributeIds, resolveNodeId, TimestampsToReturn} = require("node-opcua");
...
async.series([
    ...
    function(callback) {
        the_session.read({nodeId: "ns=1;b=1020FFAA", attributeId: AttributeIds.Value}, (err, dataValue) => {
            if (!err) {
              // console.log(" MyVariable1 = ", dataValue.value.value.toString());
              console.log(" MyVariable1 = ", dataValue.value.value);
              // console.log(" MyVariable1 = ", dataValue.statusCode.toString());
            }
            // callback(err);
          });
  ...
  the_session.write(nodesToWrite, function(err,statusCode) {
       if (!err) {
...
// const itemToMonitor = {
//     nodeId: resolveNodeId("ns=1;i=1001"),
//     attributeId: AttributeIds.Value
//   };
